...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        if (isset($data['name'])) {
            ...
            // Prepare a statement to search for the staff by name, which can be either firstName or lastName
            $stmt = $db->prepare("SELECT * FROM staff WHERE firstName LIKE :name OR lastName LIKE :name");
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $response->error = "No staff found with the provided name.";
            }
        } else {
